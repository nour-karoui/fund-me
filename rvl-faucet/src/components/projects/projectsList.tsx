...
export const ProjectsList: React.FC<{projectsFactory: any, handleProjectChange: any}> = ({projectsFactory, handleProjectChange}) => {
    ...
    const [avaiblableProjects, setAvailableProjects] = React.useState<string[]|null>(null);
    // const projects = [
    //     {name: 'Webipie'},
    //     {name: 'Ariana'},
    ...
    return (
        ...
        >
            ...
            {avaiblableProjects?.map((project, i) => <Box onClick={() => handleProjectChange(project)} key={i} borderRadius='lg' p={4} _hover={{ color: "#F8F4EA", background: "#439A97", fontWeight: 'semibold', cursor: "pointer"}}>{project}</Box>)}
        </Box>
        // <Center mt={3} pt={10}>
